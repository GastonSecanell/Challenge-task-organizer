...
import { Search, ArrowUpDown, ArrowUp, ArrowDown, ChevronDown } from 'lucide-vue-next'
...
const hasActiveFilters = computed(() => {
  return Boolean(props.search) ||
    Boolean(props.filters?.estado) ||
    Boolean(props.filters?.prioridad_id) ||
    Boolean(props.filters?.fecha_vencimiento)
})

const effectiveTotal = computed(() => (props.total > 0 ? props.total : props.items.length))
const start = computed(() => (props.items.length === 0 ? 0 : (props.page - 1) * props.perPage + 1))
const end = computed(() => Math.min(start.value + props.items.length - 1, effectiveTotal.value))
...
<template>
  <div class="mt-6 overflow-hidden rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)]">
    <TareasTableToolbar
      :search="search"
      :filters="filters"
        ...
        <table class="w-full min-w-[860px] border-collapse text-sm">
          <thead class="sticky top-0 z-20 bg-[var(--bg-surface)] text-xs">
            <tr class="border-t border-[var(--border-default)]">
              <th class="px-4 py-3 text-left font-medium text-[var(--text-secondary)]">
                <button type="button" class="inline-flex items-center gap-1 transition hover:text-[var(--accent)]" @click="$emit('sort', 'titulo')">
                  <span>Título</span>
              ...
              <th class="px-4 py-3 text-left font-medium text-[var(--text-secondary)]">
                <button type="button" class="inline-flex items-center gap-1 transition hover:text-[var(--accent)]" @click="$emit('sort', 'estado')">
                  <span>Estado</span>
              ...
              <th class="px-4 py-3 text-left font-medium text-[var(--text-secondary)]">
                <button type="button" class="inline-flex items-center gap-1 transition hover:text-[var(--accent)]" @click="$emit('sort', 'fecha_vencimiento')">
                  <span>Vencimiento</span>
                  <component :is="iconFor('fecha_vencimiento')" class="h-3.5 w-3.5" />
                </button>
              </th>
          ...
          <tbody :class="loading ? 'pointer-events-none opacity-40' : ''">
            <tr v-if="!loading && items.length === 0">
              <td colspan="5" class="px-6 py-16 text-center text-sm text-[var(--text-secondary)]">
